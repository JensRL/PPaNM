...
public class quad{
	...
	public static double vt_integrator(
		Func<double,double> f, 
		double a, //Start
		double b, //Stop
		double delta = 0.001, //Absolute accuracy
		...
		double f3 = System.Double.NaN)
		{
		Func<double,double> CCTrans;
		if(a==-1 && b==1){
			CCTrans = (x => f(Cos(x))*Sin(x));
		}
		else{
			CCTrans = (x => f((a+b)/2+(b-a)/2*Cos(x))*Sin(x)*(b-a)/2);
		}
		return integrator(CCTrans, 0, PI);
	}
